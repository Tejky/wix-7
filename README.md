...
- Pinned `VCToolsVersion=14.44.35207` and `PlatformToolset=v143` in `src/Directory.Build.props` and passed them explicitly to `msbuild` from every `*.cmd` build script (`api.cmd`, `burn.cmd`, `dtf.cmd`, `ext.cmd`, `internal.cmd`, `libs.cmd`, `setup.cmd`, `tools.cmd`, `wix.cmd`), so the build resolves a consistent C++ toolset instead of whatever the installed VS Insiders build happens to default to.
