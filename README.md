...
- `WixNativeSdkLibraryToolset` in `src/Directory.Build.props` is also `v145`. Several test projects (`BalUtilUnitTest`, `BextUtilUnitTest`, `BurnUnitTest`, `WixStdFnUnitTest`) use this property to build the path to an already-built `dutil.lib`/`balutil.lib` when linking, so it has to match wherever those libs actually land.
- The native `.nuspec` files that package the toolset's own `.lib`/`.dll` output (`wcautil`, `dutil`, `wixstdfn`, `bextutil`, `WixToolset.BootstrapperApplicationApi`) reference that build output by its `v145` output folder, for the same reason.
